  ...
  return (
    <div className="aq-page mx-auto max-w-6xl px-4 py-10 md:px-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={logout} className="flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-medium hover:bg-secondary">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[220px_1fr]">
        <nav className="flex gap-2 lg:flex-col">
          <TabButton active={tab === "orders"} onClick={() => setTab("orders")} icon={Package}>Orders</TabButton>
          <TabButton active={tab === "addresses"} onClick={() => setTab("addresses")} icon={MapPin}>Addresses</TabButton>
          <TabButton active={tab === "profile"} onClick={() => setTab("profile")} icon={User}>Profile</TabButton>
        </nav>
          ...
          <div className="mt-3 space-y-1.5">
            {o.items.map((i, idx) => (
              <div key={idx} className="flex justify-between text-sm">
                <span className="text-muted-foreground">{i.productName} · {i.size} × {i.quantity}</span>
                <span className="aq-nums text-foreground">{formatCurrency(i.lineTotal)}</span>
              </div>
            ))}
          </div>
          <div className="mt-3 flex justify-between border-t border-border pt-3">
            <span className="text-sm font-medium text-foreground">Total ({o.paymentMethod === "cod" ? "COD" : "Online"})</span>
...
function AddressesTab() {
  ...
  return (
    <div>
        ...
        {(customer.addresses || []).map((a) => (
          <div key={a.id} className="rounded-2xl border border-border bg-card p-5">
            <div className="flex items-center justify-between">
              ...
              {a.isDefault && <span className="rounded-full bg-gold/15 px-2 py-0.5 text-[10px] font-semibold text-gold-dark">Default</span>}
            </div>
            <p className="mt-2 text-sm text-muted-foreground">{a.fullName} · {a.phone}</p>
            <p className="text-sm text-muted-foreground">{a.line1}{a.line2 ? `, ${a.line2}` : ""}, {a.city} {a.state} - {a.pincode}</p>
            <button onClick={() => remove(a.id)} className="mt-3 flex items-center gap-1 text-xs text-destructive">
      ...
      {!showForm ? (
        <button onClick={() => setShowForm(true)} className="mt-5 flex items-center gap-2 rounded-full border border-border px-5 py-2.5 text-sm font-medium hover:bg-secondary">
        ...
        <div className="mt-5 rounded-2xl border border-border bg-card p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <input placeholder="Label (Home/Office)" value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} className={inp} />
            <input placeholder="Full name" value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} className={inp} />
            ...
            <input placeholder="Address line 1" value={form.line1} onChange={(e) => setForm({ ...form, line1: e.target.value })} className={cn(inp, "sm:col-span-2")} />
          ...
          <div className="mt-4 flex gap-3">
            <button onClick={save} disabled={addAddr.isPending} className="rounded-full bg-navy px-5 py-2 text-sm font-semibold text-white disabled:opacity-50">
              {addAddr.isPending ? "Saving…" : "Save"}
            </button>
            <button onClick={() => setShowForm(false)} className="rounded-full border border-border px-5 py-2 text-sm">Cancel</button>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="max-w-lg rounded-2xl border border-border bg-card p-6">
      ...
      <div className="space-y-3">
        ...
        <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone" className={inp} />
        <div className="border-t border-border pt-3">
          <p className="mb-2 text-xs font-medium text-muted-foreground">Change password (optional)</p>
          <input value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} type="password" placeholder="Current password" className={inp} />
          <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="New password" className={cn(inp, "mt-3")} />
        </div>
      </div>
      <button onClick={save} disabled={update.isPending} className="mt-5 flex items-center gap-2 rounded-full bg-navy px-6 py-2.5 text-sm font-semibold text-white disabled:opacity-50">
